...
docs/longdev/<任务>/
  ...
  reviews/                独立审查
  gates/                  主会话 gate
  evidence/               可携带的验证摘要和 manifest
```

原始日志、图片、补丁与副本在项目 `.work/longdev/<任务>/`。持久目录随阶段提交，交接包含提交 trailer/SHA 和尚未提交/未同步事项。换机器先获取包含记录的提交；缺少本机 `.work/` 时核对证据缺口，必要时重验。新版入口先执行启动迁移；旧 `.claude/longdev/`、`.claude/autopilot/` 仅作保护副本，不重新导入或恢复写入。
...
3. 子会话只回传 OCR/视觉观察、图片路径、哈希和不确定项；原图保留在项目 `.work/longdev/<任务>/evidence/`，持久记录保存路径、哈希和观察摘要。
